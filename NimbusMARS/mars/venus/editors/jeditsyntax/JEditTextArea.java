...
//       }
    ...
    public void updateScrollBars() {
        if (vertical != null && visibleLines != 0) {
            ...
            lineNumbersVertical.setValues(firstLine * height, visibleLines * height, 0, totalLines * height);
...
// protected members
    ...
    public String getLeadingWhiteSpace() {
        ...
        if (lineLength > 0) {
            ...
            int start = 0;
            int colInd = text.indexOf(':');
            int hashInd = text.indexOf('#');
            if (colInd > 0 && (hashInd == -1 || colInd < hashInd)) {
                int dLoc = getText().indexOf(".data");
                if (getText().indexOf(".text") > dLoc) {
                    dLoc = -1;
                }
                if ((dLoc == -1 || getCaretPosition() < dLoc) && mars.assembler.TokenTypes.isValidIdentifier(text.substring(0, colInd))) {
                    start = colInd+1;
                    indent = new String(new char[colInd+1]);
                    indent = indent.replace('\0', ' ');
                }
            }
    ...
    private final ItemMouseColorChange imcc = new ItemMouseColorChange();
    ...
    private void applySyntaxSensitiveHelp() {
        ...
        if (helpItems != null) {
            ...
            for (int i = 0; i < helpItems.size(); i++) {
                ...
                menuItem.addMouseListener(imcc);
    ...
    private class ItemMouseColorChange extends MouseAdapter {
        private final Color selColor = new Color(0xe6e6e6);
        private final Color regColor = new Color(0);
        public void mouseEntered(MouseEvent e) {
            JMenuItem item = (JMenuItem) e.getSource();
            item.setForeground(selColor);
        }
        public void mouseExited(MouseEvent e) {
            JMenuItem item = (JMenuItem) e.getSource();
            item.setForeground(regColor);
        }
    }
    ...
    private class PopupHelpActionListener implements ActionListener {
        ...
        // Completion action will insert a space character following the
        // completed instruction mnemonic.
