        ...
        if (c != KeyEvent.CHAR_UNDEFINED && (((modifiers & KeyEvent.ALT_MASK) == 0) || System.getProperty("os.name").contains("OS X"))) {
            if (c >= 0x20 && c != 0x7f) {
                ...
                if (evt.getKeyChar() == ':') {
                    ...
                    String line = textArea.getLineText(textArea.getCaretLine()).trim();
                    line = line.substring(0, textArea.getCaretPosition() - textArea.getLineStartOffset(textArea.getCaretLine()) - 1);
                    boolean validIdentifier = mars.assembler.TokenTypes.isValidIdentifier(line);
                    if (validIdentifier) {
                        int dLoc = textArea.getText().indexOf(".data");
                        if (dLoc == -1 || textArea.getCaretPosition() < dLoc) {
                            executeAction(INSERT_TAB, evt.getSource(), "\t");
                        }
                    }
                }
            }
